...
async function deleteTask(id) {
    await taskStore.deleteTaskItem(id);
    getAllTasks();
}
/* 7.3 */
async function editTask(id, title, description) {
    await taskStore.editTaskItem(id, title, description);
    getAllTasks();
}
...
// 4. An async function is needed to get all of the tasks stored within the supabase database, This function needs to be called within the setUp script in order to run within the first instance of this component lifecycle.
